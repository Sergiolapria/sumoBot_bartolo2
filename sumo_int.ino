...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(pin_right),ISR_izquierda,LOW);
  attachInterrupt(digitalPinToInterrupt(pin_left),ISR_derecha,LOW);
  sensor.init();
...
void ISR_derecha(){
  ...
  Serial.println("INTERRUPCION DERECHA");
}
void ISR_izquierda(){
  giro_derecha(500);
  Serial.println("INTERRPCION IZQUIERDA");
}
void loop() {
  scan_laser();
